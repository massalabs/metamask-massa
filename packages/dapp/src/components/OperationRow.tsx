...
export const OperationRow = ({ operation }: { operation: IOperationData}) => {
  ...
  const getOperationType = useMemo(() => {
    if ((operation.operation.content.op as ITransactionOpType).Transaction !== undefined) {
      return 'Transaction';
    } else if ((operation.operation.content.op as ICallSmartContractOpType).CallSC !== undefined) {
      return 'Smart Contract Call';
    } else if ((operation.operation.content.op as IExecSmartContractOpType).ExecuteSC !== undefined) {
      return 'Smart Contract Execution';
    } else if ((operation.operation.content.op as IRollBuyOpType).RollBuy !== undefined) {
      return 'Roll Buy';
    } else if ((operation.operation.content.op as IRollSellOpType).RollSell !== undefined) {
      return 'Roll Sell';
    }
  }, [operation]);
