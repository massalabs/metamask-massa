...
export const GenerateAccountModal = ({isOpen, onClose}: {isOpen: boolean, onClose: () => void}) => {
  ...
  return (
      ...
      >
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Are you sure you want to generate a new account ?</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
            <FormControl>
              <FormLabel>Account Name</FormLabel>
              <Input ref={initialRef} placeholder='Account Name' value={accountName} onChange={
                (evt) => {
